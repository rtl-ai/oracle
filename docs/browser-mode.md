...
- `--browser-agent-mode <on|off|current>`: control ChatGPT Agent mode before submitting the prompt. `current` leaves the UI state unchanged. When turning Agent mode on, Oracle dismisses connector safety prompts by turning connectors off.
